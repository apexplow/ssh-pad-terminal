 ...
 *  - SC-KHV-03: when sshj's transport verifier rejects a key, sshj
 *    raises [OpenHostKeyVerificationException], and our catch path
 *    translates it to the user-readable MITM warning. We pin the
 *    detection via an exception-chain walk identical to the one
 *    [SshClient] uses internally — a tight unit test of the
 *    translation contract without driving a real TCP socket.
...
class SshClientHostKeyWiringTest {
    ...
    fun sc_khv_03b_verifierRejectionWalksCauseChain() {
        // Mirror SshClient.isHostKeyMismatch: sshj 0.38 raises
        // [SSHException] when the verifier returns false; we identify
        // it by the cause-chain class name. (Older sshj versions exposed
        // a dedicated OpenHostKeyVerificationException; 0.38 folds it
        // into SSHException. The detection logic in SshClient walks the
        // cause chain looking for either class name.)
        val sshjEx = SSHException("Host key verification failed")
        val wrapped = RuntimeException("connect failed", sshjEx)
        val doubled = RuntimeException("top", wrapped)
        assertTrue(
            "sshj SSHException must be detected as a host-key mismatch",
