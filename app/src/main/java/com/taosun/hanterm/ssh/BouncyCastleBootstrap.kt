 ...
 * ## Why we ship our own
 *
 * Android's historical bundled "BC" provider on API 29 (Android 10) was
 * BouncyCastle 1.62, which predates Ed25519 (`java.security.spec.EdECPrivateKeySpec`
 * didn't land in BC until 1.68). minSdk is now 36 (Issue #19), but we still
 * ship and register our own BC — SSHJ 0.40+ routes Ed25519 through its
 * internal [net.schmizz.sshj.common.Ed25519KeyFactory] which calls
 * `KeyFactory.getInstance("Ed25519")`, and we must not depend on whatever
 * the OEM's system "BC" happens to be.
 *
