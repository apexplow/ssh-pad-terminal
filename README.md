...
| **Sprint 3.7** tmux session 切换器(GEARS Module 19) | ✅ 完成 | Module 16 的 SnippetPanel 在 pad 上占用大块横向画布、利用率却不高(用户常用命令实际上靠 shell history);改成更契合 pad 形态的 tmux session 切换器。新增 `terminal/TmuxSession.kt`(数据类) + `terminal/TmuxSessionParser.kt`(纯解析器,sentinel-bracketed 提取) + `terminal/TmuxSessionSource.kt`(probe 注入 + 屏幕 buffer 读取 + smart-switch 命令构造),`TermuxViewBridge.currentEmulator()` + `TerminalView.currentEmulator()` 暴露 emulator 引用,`TerminalPane.onEmulatorChanged` 把引用向上 publish;`ui/TmuxDrawer.kt` 用 `Box` + scrim + `AnimatedVisibility` 自定义右侧抽屉(Material3 的 `ModalNavigationDrawer` 只支持 start edge);切换命令 `tmux switch-client -t X 2>/dev/null \|\| tmux attach -t X` —— shell 的 `\|\|` 自动按"当前在 tmux 内/外"分流,sentinel + `2>/dev/null` 把脏输出清掉。删除 `ui/SnippetPanel.kt` + `ui/SnippetPayload.kt`(数据 `SnippetStore` 保留 dormant,SharedPreferences 内容不丢);`HanTermApp` 的 `snippetStore` / `showSnippetPanel` 改为 `tmuxEmulator` / `tmuxSource` / `showTmuxDrawer`。`TmuxSessionParserTest` 8 case(纯 JUnit)+ `TmuxSessionSourceTest` 7 case(Robolectric,真实 `TerminalEmulator` round-trip),无 `@Ignore` / 无 `assumeTrue` 门控。详见 [`docs/GEARS_SPEC.md` Module 19](docs/GEARS_SPEC.md#module-19-tmux-session-switcher-sprint-37) |
| Sprint 4+ 主机管理 / SFTP / Mosh | 📋 远期 | 见 [路线图](#路线图) |

v1.0 + Sprint 3 已在平板上**配置主机 / 保存密码(Keystore AES-256-GCM) / 导入并加密私钥(AES-256-GCM → `filesDir/keys/`) / known_hosts TOFU 校验 / debug log + auth 诊断 gating / 通过音量键调字号 / 跑 vim 和 nano 时所有标准快捷键 / 双指翻页看 scrollback 历史 / 平板横屏双栏布局 / 右侧抽屉列出 tmux sessions 一键切换 / Disconnect 后不再误弹 "Connection Closed" 红覆盖 / 在 app 内看诊断日志与崩溃栈并复制 / SSH keepalive 主动探测死连接 90 s 内断开 / `PtyBridge` 把 transport 层做成可替换(下一站:mosh / 本地 shell) / `SshClient.disconnect` 并发原子,Disconnect 按钮 + writeExecutor onClose + UI error handler 三路并发安全**。剩下的是多主机列表、SFTP、Mosh、SSH-keepalive 服务器兼容性矩阵等 Sprint 4+ 工作。
...
| `SnippetStoreTest` | **10**(Sprint 3 M16;M19 中类保留 dormant,数据保留在 SharedPreferences 中以备未来 sprint 重新挂回 UI) | Robolectric | `SnippetStore` CRUD + JSON 序列化 / append-order / update 保留位置 / delete-by-id + 未知 id / 单 SharedPreferences 字段 + 三类损坏恢复 + 单行损坏隔离 + 损坏后写入修复(pin SNP-ST-01..06) |
| `TmuxSessionParserTest` | **8**(Sprint 3.7 M19) | 纯 JUnit | `TmuxSessionParser.parse(transcript)` sentinel-bracketed 提取 + 单行损坏隔离 + 列数错配丢弃 + 头哨兵/尾哨兵缺失返空 + 空白行 trim(pin TSP-04, TSP-05) |
| `TmuxSessionSourceTest` | **7**(Sprint 3.7 M19) | Robolectric + 真实 `TerminalEmulator` round-trip | `switchCommand` 单引号包裹 / 空格 / CJK UTF-8 / 末尾 `\r`(pin TSS-01..04)+ `refresh` 写 probe 一次 + 屏幕 buffer 解析往返 + 错误状态(空 sentinel、emulator 不可用)(pin TSP-01..03) |
...
- [x] 命令 Snippet(`data/prefs/SnippetStore.kt` + `ui/SnippetPanel.kt` + `ui/SnippetPayload.kt`,右上 IconButton 打开,Material3 `ModalBottomSheet` + LazyColumn + Add/Edit/Delete)—— 见 [`docs/GEARS_SPEC.md` Module 16](docs/GEARS_SPEC.md#module-16-command-snippets-sprint-3-s2);`SnippetStoreTest` 10 个 + `SnippetPayloadTest` 4 个(Sprint 3.7 Module 19 把 UI 部分删除,数据层 `SnippetStore` 保留 dormant;详见 Module 19)
- [x] tmux session 切换器(`terminal/TmuxSession.kt` + `terminal/TmuxSessionParser.kt` + `terminal/TmuxSessionSource.kt` + `ui/TmuxDrawer.kt`,右上 IconButton 打开,自定义右侧抽屉 + sentinel-bracketed 屏幕 buffer 解析 + smart-switch 命令)—— 见 [`docs/GEARS_SPEC.md` Module 19](docs/GEARS_SPEC.md#module-19-tmux-session-switcher-sprint-37);`TmuxSessionParserTest` 8 个 + `TmuxSessionSourceTest` 7 个
