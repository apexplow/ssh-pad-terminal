...
**IO scope**: `ConnectionRuntime` 内部 `CoroutineScope(SupervisorJob + ioDispatcher)` 承载 adapter 三路协程(outbound / inbound / watchdog,硬编 `Dispatchers.IO`,**不能**挂在虚拟时间 test dispatcher 上 —— `BufferedPtyBridge.Endpoint.read()` 用阻塞 `LinkedBlockingQueue.take()`,见 `TeardownState` 段 memory note)。`disconnect()` 的 teardown 主体不走 `ioScope.launch`,而是 `withContext(ioDispatcher) { ... }` 同步在 caller 协程里 hop 到 `Dispatchers.IO`(见 `TeardownState` 段)。UI scope 取消不带走 bridge 协程;`runtime.dispose()` 才 cancel IO scope —— 但 post-`dispose()` 的 `disconnect()` 仍能跑完 teardown,因为 `withContext(ioDispatcher)` 只引用 `ioDispatcher`,不依赖 `ioScope` 还活着。
