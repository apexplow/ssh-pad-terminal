...
class HanTermAppViewModel(
    ...
    fun onSessionClosed(reason: String, closeReason: com.apexplow.hanterm.ssh.SessionCloseReason) {
        viewModelScope.launch {
            runtime.disconnect(
                userInitiated = false,
...
private fun formatCloseMessage(
    closeReason: com.apexplow.hanterm.ssh.SessionCloseReason,
    fallback: String,
): String = when (closeReason) {
    is com.apexplow.hanterm.ssh.SessionCloseReason.TransportError ->
        "Network error: ${closeReason.message}"
    is com.apexplow.hanterm.ssh.SessionCloseReason.SinkError ->
        "Internal error: ${closeReason.message}"
    com.apexplow.hanterm.ssh.SessionCloseReason.IdleTimeout ->
        "Session ended due to inactivity."
    com.apexplow.hanterm.ssh.SessionCloseReason.RemoteEof ->
        "Remote host closed the connection."
    com.apexplow.hanterm.ssh.SessionCloseReason.UserInitiated -> fallback
}
