...
| TIC-DS-04 | When `deleteSurroundingText` is invoked, `TerminalInputConnection` shall read `userInImeContext` to decide THIS call's routing and then immediately set it to `false` (one-shot consume), so that a later `deleteSurroundingText` call — not itself preceded by a fresh `setComposingText`/`commitText`/`finishComposingText` — reaches SSH as a real DEL. | **Sprint 3.5 bug fix**: prior to `TerminalInputConnectionTest.test_deleteSurroundingText_afterCommit_onlyTheFirstDelIsSuppressed`, the latch was only ever set to `true` and never reset anywhere in the codebase (verified via `git log --all -p`) — the kdoc claimed a "reset after successful DEL" that lived in a structurally unreachable branch (you can only reach the write-DEL branch when the latch already reads `false`). Net effect: after the user's first pinyin composition of a `TerminalInputConnection`'s lifetime, every later on-screen-keyboard backspace was silently swallowed and never reached SSH again. Fixed by moving the reset to the top of the method so it consumes the latch on every call regardless of branch. |
...
| TV-SB-02 | Given a scrollback-active `ScrollbackController` and a `MotionEvent` with `actionMasked == ACTION_UP` whose centroid Y differs from the initial POINTER_DOWN centroid by more than `lineSpacing * mRows / 2`, the controller shall invoke `innerView.doScroll(move, ±mRows)` and the inner view's `mTopRow` shall change by exactly one page in the indicated direction. | Page scroll threshold + doScroll reflection. **Superseded by TV-SB-02b (2026-07-10) — the legacy half-screen threshold was lowered to quarter-screen and supplemented with a fling-velocity fallback and a peak-displacement check. The original wording stays for audit traceability.** |
| TV-SB-02b | Given a scrollback-active `ScrollbackController` and an `ACTION_UP`, the controller shall invoke `doScroll(move, ±(mRows - 1))` — and the inner view's `mTopRow` shall change by the requested amount (which may be smaller if the user is near the top or bottom of the scrollback) — when ANY of: net `|dy| > lineSpacing * mRows / 4`, peak displacement across the gesture `> lineSpacing * mRows / 4`, or `|velocityY| > 1_500 px/s` at finger-up. The banner hint shall display the **actual rows moved**, not "一页" (e.g. "↑ 已向上翻 3 行" when only 3 rows of headroom remain; "已到顶部" when zero). | Supersedes TV-SB-02: addresses the user-reported "trigger insensitive / inaccurate line count" bugs. The one-row overlap (`mRows - 1` rather than `mRows`) matches the alt-buffer cursor-key fallback (`sendAltBufferPageScroll` uses `(emulator.mRows - 1).coerceAtLeast(1)`). |
| TV-SB-03 | Given a scrollback-active `ScrollbackController` and a `MotionEvent` with `actionMasked == ACTION_UP` whose centroid Y differs from the initial centroid by less than the threshold, no `doScroll` call shall happen; `mTopRow` is unchanged. | Sub-threshold swipe is a no-op. **Superseded by TV-SB-03b (2026-07-10) — the threshold is now quarter-screen, and either the peak or the velocity can independently fire the page flip.** |
| TV-SB-03b | Given a gesture that crosses neither the quarter-screen spatial threshold (net OR peak) NOR the 1_500 px/s fling threshold at finger-up, the controller shall publish the hint "滑动距离不够（需超过 1/4 屏）" and shall not invoke `doScroll`. | Spatially-small but velocity-rich flicks still fire (TV-SB-02b); spatially-poor flicks still get an honest hint instead of silently no-op. |
...
| TV-SB-07 | Given a `ACTION_UP` while `mTopRow` is already at the top of the available scrollback (so the clamp yields `amount = 0`), the controller shall publish the hint "已到顶部" without invoking `doScroll`. Mirror: page-down at the live view shall publish "已在最底部" and clear `isInScrollback`. | Clamps emit honest hints; no silent no-ops at boundaries. |
...
|---|---|---|---|
| Module 1: IME pipeline | 14 (TIC-*) + 2 (TE-*) | `TerminalInputConnectionTest.kt` (19 tests) | ✅ Core flow covered; `TIC-DS-04` (latch reset) pinned by `test_deleteSurroundingText_afterCommit_onlyTheFirstDelIsSuppressed` (Sprint 3.5 — this test also caught and fixed a real bug, see TIC-DS-04 row above) |
...
| Module 3: View | 15+ (TV-EM-*, TV-PTY-*, TV-AB-*, TV-FS-*) | `TerminalViewLayoutTest.kt` (3) + `AltBufferScrollCrashGuardTest.kt` (9) | ✅ Alt-buffer guard (TV-AB-01..05) and layout re-measure (TV-LY-01..02) pinned; `TV-FS-01` (font-size idempotency) pinned by `setTextSize_sameValueAsCurrent_isNoOpAndDoesNotTouchRenderer` (Sprint 3.5) |
...
| **Total** | **~316 GEARS specs** | **169/178 green** (0 `@Ignore` + 11 `@Assume`, as of Sprint 3.5 — `TIC-DS-04` and `TV-FS-01` newly pinned) | |

### Test inventory by file (2026-07-09 snapshot — re-verified against `app/src/test/` source, not just prior doc claims)
...
| `ssh/auth/PublicKeyAuthProviderTest.kt` | 5 | 0 | **Sprint 3.5** (`e4487b2`) SSHJ 0.38→0.40 upgrade un-Ignored the 2 Ed25519 cases (was 3 `@Test` active / 2 `@Ignore`) via a rewritten `writeOpenSshPem` fixture that encodes real OpenSSH-v1 wire format through BC's `OpenSSHPrivateKeyUtil` instead of relying on `JcaMiscPEMGenerator`'s PKCS#8 output (which SSHJ 0.40's `PKCS8KeyFile` now hard-rejects for the Ed25519 OID) |
| `ssh/auth/PublicKeyAuthProviderEncryptedTest.kt` | 5 | 0 | Sprint 2.5 S2 release-only encrypted PEM path (all 5 `@Assume`-gated) |
...
| `data/crypto/EncryptedPrivateKeyStoreTest.kt` | 8 | 0 | Sprint 2.5 S2 (6 of the 8 are `@Assume`-gated — Robolectric's AndroidKeyStore is a stub) |
...
8. ~~**`TIC-DS-04` (latch reset)** + **`TV-FS-01` (font-size idempotency)`**~~ — **both closed in Sprint 3.5**. `TIC-DS-04` in `TerminalInputConnectionTest.test_deleteSurroundingText_afterCommit_onlyTheFirstDelIsSuppressed` (this one also caught and fixed a real latent bug — see the TIC-DS-04 row above). `TV-FS-01` in `TerminalViewLayoutTest.setTextSize_sameValueAsCurrent_isNoOpAndDoesNotTouchRenderer`. `KM-CTL-04` (Ctrl+ESC) was closed by the Sprint 2.5+ vim/nano KeyMapper refactor (`KeyEventRoutingTest.test_ctrlEscape_writesEscByte`).
