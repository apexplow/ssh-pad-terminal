...
| **Sprint 3.7** tmux session 切换器(GEARS Module 19) | ✅ 完成 | 右侧抽屉通过同一已认证 SSH 连接上的独立 exec channel 静默查询 `tmux list-sessions`，刷新对当前 PTY 零写入；固定超时、并发排空 stdout/stderr、限长并区分 no-server / PATH / transport 错误。Bash/Zsh shell integration 通过终端标题回调上报 READY/BUSY/tmux 状态：agent/TUI 中只读列表、不注入命令；tmux 内隐藏侧栏并显示按实际 prefix detach 的“退出 tmux”按钮。安装脚本由用户复制后手动执行，应用不自动改远端 dotfiles。详见 [`docs/GEARS_SPEC.md` Module 19](docs/GEARS_SPEC.md#module-19-tmux-session-switcher-sprint-37) |
...
| `TmuxSessionParserTest` | Sprint 3.7 M19 | 纯 JUnit | 独立 exec stdout 解析、稳定 session id、`|` 名称、控制字符清理、坏行隔离 |
| `TmuxSessionSourceTest` | Sprint 3.7 M19 | 纯 JUnit | refresh 对 PTY 零写入、连续刷新取新结果、状态码映射、串行查询、attach/detach 字节 |
| `SshjRemoteCommandExecutorTest` | tmux 静默查询 | 纯 JUnit | 双流排空、输出上限、超时/取消主动关 channel、关闭后拒绝 |
| `ShellIntegrationStateTest` / `TmuxDrawerUiTest` | tmux 状态 UI | JUnit + Robolectric Compose | 标题协议、prefix 编码、unknown/BUSY/READY 行为门控 |
